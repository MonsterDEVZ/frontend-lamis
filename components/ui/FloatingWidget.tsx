  ...
  return (
    <div className="fixed right-4 z-50 flex items-end gap-3 bottom-15 lg:right-5 lg:gap-4 lg:bottom-20">
      <div className="relative">
        {isOpen && (
          <div className="absolute bottom-14 right-0 flex flex-col items-center gap-2 lg:bottom-13 lg:gap-3">
            {actionButtons.map((item) => (
              <button
                key={item.name}
                className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-green-100 text-white shadow-lg transition-colors duration-200 hover:bg-green-700 lg:h-[50px] lg:w-[50px]"
        ...
        <button
          className="relative flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-green-100 text-white shadow-xl transition-colors duration-300 hover:bg-green-700 lg:h-[50px] lg:w-[50px]"
          onClick={toggleMenu}
        >
          {isOpen ? (
